		...
		try {
			...
			int tocall = (int) ((state.getInitjetton() * (prob
					* activeIncludingSelf - 1))
					/ activePlayers * 0.8);
			int maxbet = (int) (state.getInitjetton() / 1.5);
			int prebet = 0;
			try {
			...
			if (tocall < hightocall) {
				...
				if (base == 0) {
					if(state.currentState!=State.baseState)
						State.raisebet = (int)(0.6*(2+state.callnum) * state.bigblindbet+State.raisebet*0.4);
					return State.raise;
				}
				...
				if ( (state.raisenum < 2)) {
					if(state.currentState!=State.baseState)
						State.raisebet =(int) (0.6*(2+state.callnum+(aveval/base-myval)*3) * state.bigblindbet+State.raisebet*0.4);
					return State.raise;
				} else
					return State.call;
			}
