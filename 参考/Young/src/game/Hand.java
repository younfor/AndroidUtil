...
//	}
	...
	public static enum HandCategory {
		NO_PAIR, PAIR, TWO_PAIR, THREE_OF_A_KIND, /*˳��*/STRAIGHT, /*ͬ��*/FLUSH, /*��«��������*/FULL_HOUSE, FOUR_OF_A_KIND, STRAIGHT_FLUSH;
	}

	private static final int BOT_SHIFT = 16;
	private static final int TOP_SHIFT = BOT_SHIFT + 4;//20
	private static final int VALUE_SHIFT = TOP_SHIFT + 4;//24
	...
	private static final int ARRAY_SIZE = 0x1FC0 + 1; /*8128+1=8129*/// all combos of up to 7 of LS 13 bits on
	 ...
	 */
	public static final int NO_8_LOW = STRAIGHT_FLUSH + (1 << VALUE_SHIFT);/*8<<24+1<<24*/
	...
	public static int hand5Eval(long hand) {
		...
		case 5: /* flush and/or straight, or no pair */
			if ((i = straightValue[ranks]) == 0)//==0，不是顺子
				i = ranks;
			if (c != 0) { /* if any clubs... */
	...
	public static int HandEvaluate(Card ...cards){
		long hand =0L;
		for(int i=0;i<cards.length;i++){
			hand|=encode(cards[i]);
		}
		return cards.length==5?hand5Eval(hand):(cards.length==6?hand6Eval(hand):hand7Eval(hand));
	}
	/*
	 * test
	 *皇家 同花顺 135135232
   ...
   *一对：1010234 100663552
    ...
    *葫芦     999JK  42994176
	...
	public static void main(String []args){
		...
		cards[0]=new Card("DIAMONDS 9");
		cards[1]=new Card("CLUBS 9");
		cards[2]=new Card("CLUBS 9");
		cards[3]=new Card("CLUBS Q");
		cards[4]=new Card("CLUBS K");
//		cards[5]=new Card("DIAMONDS 9");
//		cards[6]=new Card("DIAMONDS 8");
		for(int i=0;i<cards.length;i++){
			hand|=encode(cards[i]);
		}
		System.out.println(hand5Eval(hand));
	}
}
