...
public class CleverBot implements Bot {
	...
	public int getAction() {
		try {
			...
			boolean allin=false;
			if(state.getInitjetton()>75*state.bigblindbet && me.getGold()>40*state.bigblindbet)
			{
				allin=true;
			}
			...
			if (tocall < hightocall) {
				...
				if (prob1 + prob2 >= prob3 &&(level>=1||base==0 ||myval<aveval/base+1)) {
					{
						...
						if((prob<0.4||prob<maxprob-0.15)&&highbet>5*state.bigblindbet&&(maxprob==0||maxprob>prob))
						{
							debug("doubi fold");
							return State.fold;
						}
				...
				{
					if(maxprob>prob&&state.raisenum>0)
					{
