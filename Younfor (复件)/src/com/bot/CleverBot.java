...
public class CleverBot implements Bot {
	...
	public int getBestAction(State state, long time) {
		...
		if (state.currentState == State.baseState) {
			if((!level1())&&hightocall-state.getPrebet()>15*state.bigblindbet&&state.getJetton()<150*state.bigblindbet)
			{
				debug("too big handcards");
				return State.fold;
			}
			if((!level1())&&(!level2())&&hightocall-state.getPrebet()>8*state.bigblindbet&&state.getJetton()<40*state.bigblindbet)
			{
				debug("too big handcards");
				return State.fold;
			}
	...
	public int getBL() {
		if (state.myloc == State.bigblind) {
			...
			if (level3()) {
				if(isFoldAll()||isCall1()||isCall2())
				{
					State.raisebet=(2+state.callnum)*state.bigblindbet/2;
					return State.raise;
				}
				if(isRaise1Call0())
				{
	...
	public int getAction() {
		try {
			...
			if (tocall < hightocall) {
				...
				{
					State.raisebet=(int)((state.getJetton()*size*state.totalpot/(state.totalpot+state.getJetton()))*prob*prob/((activeIncludingSelf+state.raisenum)*maxprob));
				}
				//test
				if(state.currentState==State.flopState||state.currentState==State.turnState||state.currentState==State.riverState)
				{
					...
					if(prob>0.8)
					{
						...
						{
							if(state.raisenum>0&&prob<maxprob&&raisemuch&&raisefold)
							{
								debug("raisemuch");
								return State.fold;
							}
						...
						{
							if(state.currentState==State.flopState&&prob<maxprob&&raisemuch&&raisefold)
							{
								debug("raisemuch 0.8");
								return State.fold;
							}
