...
public class CleverBot implements Bot {
	...
	public boolean level5() {
		...
		if (isSameSuit()) {
			if ( is(13,9)||is(9,8)||is(14, 9) || is(14, 8) ||is(14, 7) || is(14, 6)) {
				debug("level 5");
				return true;
			}
		}
		if (is(13, 11) || is(13, 10) || is(12, 11) ) {
			debug("level 5");
			return true;
		}
		//5-29 23:03
	...
	public int getAction() {
		try {
			...
			if (tocall < hightocall) {
				...
				if (prob1 + prob2 >= prob3 &&(level>=1||base==0 ||myval<aveval/base+1)) {
					{
						...
						if(state.currentState==State.turnState&&prob<0.5&&highbet>10*state.bigblindbet)
						{
							debug("lose turn fold");
							return State.fold;
						}
				...
				{
					//allin
					if(level==2&&prob>0.60&&(maxprob<prob+0.5&&maxprob!=0)&&state.raisenum==0)
					{
						debug("all_in");
						State.raisebet=20*state.bigblindbet;
						return State.raise;
					}
					if(prob>0.9)
					{
						...
						if(state.raisenum==0&&maxprob>0.8)
							return State.call;
						if(state.raisenum>0)
							State.raisebet=10*state.bigblindbet;
						return State.raise;
					}
					if(prob>0.8)
					{
						...
						else if(state.raisenum==0&&maxprob>0.8)
							return State.call;
					...
					if(prob>0.7)
					{
						...
						if(state.raisenum>0)
							State.raisebet=3*state.bigblindbet;
						else if(state.raisenum==0&&maxprob>0.7)
							return State.call;
						else if(state.raisenum==0)
							State.raisebet=state.bigblindbet;
						return State.raise;
					...
					if(prob>0.6)
					{
						...
						if(maxprob<0.8&&me.getGold()==50*state.bigblindbet
								&&me.getJetton()>50*state.bigblindbet&&oppojetton<50*state.bigblindbet)
						{
							debug("small all_in");
							return State.all_in;
						}
				...
				if(state.currentState==State.turnState&&maxprob>prob&&state.raisenum>0)
				{
					debug("avoid being fished");
					return State.call;
				}
				...
				if ( (state.raisenum < 2)) {
					...
					if(state.currentState==State.flopState&&prob>0.6&&level==2)
					{
						State.raisebet=3*state.bigblindbet;
						return State.raise;
					}
					if(level==1&&prob>0.6)
					{
						...
						if(state.getInitjetton()<25*state.bigblindbet&&me.getGold()<5*state.bigblindbet)
						{
							...
							if(prob<0.6&&(state.currentState!=State.flopState)&&myval>aveval/base&&prob<maxprob)
								return State.fold;
							return State.call;
						}
						State.raisebet=2*state.bigblindbet;
				...
				if(prob<0.9&&state.raisenum>=2)
				{
					if(level==2&&prob>0.85)
					{
						debug("try try call");
						return State.call;
					}
