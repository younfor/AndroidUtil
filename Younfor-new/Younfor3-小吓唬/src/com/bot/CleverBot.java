...
public class CleverBot implements Bot {
	...
	public int getBL() {
		if (state.myloc == State.bigblind) {
			...
			if (level2()) {
				...
				{
					State.raisebet=(5+state.callnum)*state.bigblindbet;
					return State.raise;
				}
				if(isRaise1Call0())
				{
	...
	public int getAction() {
		try {
			...
			//scare
			if(highbet-prebet<=state.bigblindbet&&state.raisenum<=1&&maxprob<0.6)
			{
					debug("just a test");
					State.raisebet=state.bigblindbet;
					return State.raise;
			...
			if (tocall < hightocall) {
				...
				if(state.currentState==State.flopState)
				{
					...
					{
						debug("test skill");
						if(highbet-prebet>16*state.bigblindbet)
							return State.call;
						if(state.raisenum>1)
							State.raisebet=8*state.bigblindbet;
						else if(state.raisenum==1)
							State.raisebet=4*state.bigblindbet;
					...
					{
						debug("test skill");
						if(highbet-prebet>8*state.bigblindbet)
							return State.call;
						if(state.raisenum>1)
							State.raisebet=4*state.bigblindbet;
						else if(state.raisenum==1)
							...
							State.raisebet=1*state.bigblindbet;
						return State.raise;
					}
					if(prob>0.7)
					{
						debug("test skill");
						if(highbet-prebet>4*state.bigblindbet)
							return State.call;
						if(state.raisenum>1)
							return State.call;
						else if(state.raisenum==1)
					...
					{
						debug("big bet");
						if(highbet-prebet>4*state.bigblindbet&&state.raisenum>1)
							return State.call;
						if(state.raisenum>1)
							return State.call;
						else if(state.raisenum==1)
