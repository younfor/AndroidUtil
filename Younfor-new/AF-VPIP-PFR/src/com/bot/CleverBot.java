...
public class CleverBot implements Bot {
	...
	public int getPreAction()
	{
		...
		else if(state.myloc==State.button)
		{
			...
			if(level>6&&hightobet-prebet<=4*state.bigblindbet)
			{
				...
				if(state.myraisenum>size)
					return State.call;
				State.raisebet=(int)(1*state.bigblindbet);
				return State.raise;
			}
			...
			{
				State.raisebet=(int)(2*state.bigblindbet);
				return State.raise;
			}
		}
		//大盲位置
		else if(state.myloc==State.bigblind)
		{
			...
			if(isCall2()||(isCall1()||isFoldAll())||(isRaise1Call0()&&hightobet-prebet<=4*state.bigblindbet))
			{
				//1
				if(state.myraisenum>size)
					return State.call;
				State.raisebet=(int)(3*state.bigblindbet);
				return State.raise;
			}
		}
		//小盲位置
	...
	public int getAction() {
		try {
				if(raise())
				{
					...
					{
						if(hightobet-prebet<=4*state.bigblindbet&&prob<0.6)
						{
							debug("raise:"+State.raisebet);
							State.raisebet=(int)(5*state.bigblindbet);
							return State.raise;
						}//3
						else if(prob>0.6&&hightobet-prebet<=4*state.bigblindbet&&state.myraisenum<2)//5
						{
							//3
							State.raisebet=(int)(2*state.bigblindbet);
							return State.raise;
				...
				}else
				{
					//吓唬
					debug("吓唬");
					if((state.currentState==State.flopState||(state.currentState==State.turnState))&&hightobet-prebet<=3*state.bigblindbet)
					{
						State.raisebet=(int)(3*state.bigblindbet);
						return State.raise;
					}
	...
	public boolean raise() throws IOException
    {
		...
		int maxbet=1;
		//7
		maxbet= (int) (state.getInitjetton() / 1.45);
		prebet = 0;
		try {
			prebet = state.getPrebet();
		} catch (Exception e) {
