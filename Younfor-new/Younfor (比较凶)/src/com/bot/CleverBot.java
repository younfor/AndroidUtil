	...
	public int getAction() {
		try {
			...
			if (tocall < hightocall) {
				...
				{
					if(highbet<state.getInitjetton()/13&&foldmuch&&(isFoldAll())&&(State.turnState==state.currentState||State.riverState==state.currentState))
					{
						debug("folmuch1 raise");
						State.raisebet=state.totalpot;
						return State.raise;
					}
					if(highbet<state.getInitjetton()/10&&foldmuch&&(isCall1())&&(State.turnState==state.currentState||State.riverState==state.currentState))
					{
						debug("folmuch2 raise");
						State.raisebet=state.totalpot;
						return State.raise;
					}
					if(highbet<state.getInitjetton()/15&&foldmuch&&isCall2()&&(State.turnState==state.currentState||State.riverState==state.currentState))
					{
						debug("folmuch3 raise");
						State.raisebet=state.totalpot;
						return State.raise;
					}
				}else
				{
					if(highbet<state.getInitjetton()/18&&foldmuch&&(isFoldAll())&&(State.turnState==state.currentState||State.riverState==state.currentState))
					{
						debug("folmuch4 raise");
						State.raisebet=state.totalpot;
						return State.raise;
					}
					if(highbet<state.getInitjetton()/15&&foldmuch&&(isCall1())&&(State.turnState==state.currentState||State.riverState==state.currentState))
					{
						debug("folmuch5 raise");
						State.raisebet=state.totalpot;
						return State.raise;
					}
					if(highbet<state.getInitjetton()/20&&foldmuch&&isCall2()&&(State.turnState==state.currentState||State.riverState==state.currentState))
					{
						debug("folmuch6 raise");
						State.raisebet=state.totalpot;
						return State.raise;
					}
				}
				...
				if (prob1 + prob2 >= prob3) 
				{
					...
					if(raisefold&&maxprob==0&&(prob<0.91&&me.getGold()<4*state.bigblindbet))
						return State.fold;
					//if(raisefold&&maxprob==0)
					//	return State.fold;
					if(highbet<state.getInitjetton()/8&&foldmuch&&state.currentState==State.flopState)
					{
						debug("folmuch7 raise");
						State.raisebet=state.totalpot;
						return State.raise;
					}
					else
				...
				{
					if(highbet<state.getInitjetton()/10&&foldmuch&&state.currentState==State.flopState)
					{
						debug("folmuch8 raise");
						State.raisebet=state.totalpot;
						return State.raise;
					}
					return State.call;
				...
				{
					if(highbet<state.getInitjetton()/6&&foldmuch&&state.currentState==State.flopState)
					{
						debug("folmuch9 raise");
						State.raisebet=state.totalpot;
						return State.raise;
					}
					return State.call;
				...
				{
					if(state.getInitjetton()/10>state.totalpot&&raiseless&&foldmuch&&state.getInitjetton()>60*state.bigblindbet&&state.currentState==State.flopState)
					{
						debug("folmuch10 raise");
						State.raisebet=state.totalpot;
						return State.raise;
				...
				if(((int)size)>5) size=1;
				debug("size:"+size);
				//State.raisebet=(int)(size*tocall/(1-prob+2*maxprob));
				//if(maxprob==0)
				State.raisebet=(int)(size*tocall/(1-prob+2*maxprob));
				//else
				//	State.raisebet=(int)Math.max((int)(size*tocall/(1-prob+2*maxprob)),size*maxaverbet*prob/maxprob);
				//test
				if(state.currentState==State.flopState||state.currentState==State.turnState||state.currentState==State.riverState)
				{
					if(highbet-prebet==0&&maxprob<prob+0.2)
					{
						...
						//State.raisebet=(int)Math.min(State.raisebet, size*tocall/(1+maxprob));
