...
public class CleverBot implements Bot {
	...
	public boolean level5() {
		if(state.getInitjetton()<25*state.bigblindbet ||(state.getInitjetton()<50*state.bigblindbet&&me.getGold()<5*state.bigblindbet))
		{
	...
	public int getAction() {
		try {
			...
			if(state.getInitjetton()>100*state.bigblindbet && me.getGold()>40*state.bigblindbet)
			{
				//  4000  2000
				 level=1; 
				 maxbet= (int) (state.getInitjetton() / 2);
			}
			...
			if(hightocall-prebet>15*state.bigblindbet&&prob<0.8)
			{
				debug("big oppoent");
				return State.fold;
			}
			...
			if (tocall < hightocall) {
				...
				if (base == 0) {
					if(state.currentState!=State.baseState)
						State.raisebet = (int)(State.raisebet*0.5);
					return State.raise;
					//return State.call;
				}
				...
				if ( (state.raisenum < 2)) {
					...
					if(state.currentState!=State.baseState)
					{
						if(prob>0.9)
							...
							return State.call;
						//State.raisebet =(int) (0.5*(3+state.callnum+(aveval/base-myval)*2.5) * state.bigblindbet+State.raisebet*0.5);
					}
					//State.raisebet*=0.65;
					return State.raise;
				} else
