...
public class CleverBot implements Bot {
	...
	public int getPreAction()
	{
		...
		//6
		//新一轮
		if(state.round>1&&hightobet-prebet<=size*3*state.bigblindbet&&level>=5)
			return State.call;
		//关煞位置
	...
	public int getAction() {
		try {
				...
				{
					debug("raise");
					if(state.currentState==State.flopState||state.currentState==State.riverState)
					{
						...
						else if(prob>0.6&&hightobet-prebet<=size*4*state.bigblindbet)
						{
							//3
				...
				}else
				{
					//吓唬
					debug("吓唬");
					if((state.currentState==State.flopState||state.currentState==State.riverState)&&hightobet-prebet<=size*3*state.bigblindbet)
					{
						State.raisebet=(int)(size*5*state.bigblindbet);
						return State.raise;
					}
