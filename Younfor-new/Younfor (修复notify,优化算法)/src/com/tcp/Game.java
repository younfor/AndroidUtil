...
public class Game {
	...
	//取出堆栈
	public void popMsgStack() throws IOException
	{
		//新的一轮,只能从小盲开始
		if(state.newround)
		{
	...
	//处理消息
	public void processMsg(String data[]) throws IOException
	{
		Player p=findById(data[0]);
		...
		if(p.getLastaction()!=State.fold &&state.currentState==State.baseState&&state.newseat)
		{
		...
		if(oldbys==-1)
		{
			...
			else if((p.getLastaction()==State.raise||p.getLastaction()==State.all_in))
			{
				...
				p.actions2[state.currentState-State.baseState]=raisetype;
			}
