		...
		while((s=in.readLine())!=null)
		{
			...
			else if(s.startsWith("seat"))
			{
				...
				while(!s.startsWith("/seat"))
				{
					...
					//new 05-28 23:36
					if(findActionById(player.getPid())==null)
					{
						debug("register oppoer: "+player.getPid());
						state.oppotype.add(new PlayerAction(player.getPid()));
					}
			...
			}else if(s.startsWith("inquire")||s.startsWith("notify"))
			{
				...
				while((!s.startsWith("/inquire"))&&(!s.startsWith("/notify")))
				{
					...
					if(data[0].startsWith("total"))
					{
						...
						//new oppo 05-28 23:49
						if(p.getLastaction()==State.fold)
						{
							findActionById(p.getPid()).action[state.currentState-State.baseState].fold++;
						}
						if(p.getLastaction()==State.call||p.getLastaction()==State.check)
							findActionById(p.getPid()).action[state.currentState-State.baseState].call++;
						if(p.getLastaction()==State.raise)
							findActionById(p.getPid()).action[state.currentState-State.baseState].raise++;
						if(p.getLastaction()==State.all_in)
							findActionById(p.getPid()).action[state.currentState-State.baseState].all_in++;
