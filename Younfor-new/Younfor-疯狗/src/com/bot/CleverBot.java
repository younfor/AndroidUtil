...
public class CleverBot implements Bot {
	...
	public int getAction() {
		try {
			...
			else if(state.getInitjetton()>80*state.bigblindbet)
			{
				// 4000  0
			...
			debug("h-p:"+(hightocall-prebet)+",15BB:"+(15*state.bigblindbet));
			
			if((hightocall-prebet>15*state.bigblindbet)&&(prob<0.9))
			{
				if(level>=1&&prob>0.8)
				{
					//05-28 01:05
			...
			double aveval = 0, base = 0,minval=10;
			try{
				for (Player p : players) {
			...
			if (tocall < hightocall) {
				...
				if(base!=0)
					...
					debug("base=0");
				if(prob<0.4&&state.raisenum>0)
				{
					//05-28 01:05
					if(level==2&&prob>0.33)
					{
						debug("level 2 try doubi fold");
						return State.call;
					}
					debug("doubi fold");
					return State.fold;
				...
				if(state.currentState==State.flopState)
				{
					...
					if(prob>0.91&&level>=1&&me.getGold()>40*state.bigblindbet)
					{
						debug("all in");
						return State.all_in;
					}
				...
				if(state.currentState==State.riverState)
				{
					...
					if(prob>0.95&&level>=1)
					{
						debug("test skill allin");
						State.raisebet=(int)Math.max(20*state.bigblindbet,state.totalpot*2);
						return State.raise;
					}
					if(prob>0.9)
					{
				...
				//new 
				int mynum=findActionById(me.getPid()).action[state.currentState-State.baseState].raise;
				debug(" myraisenum:"+mynum);
				if(level<0.75&&state.raisenum>0)
				{
					for (Player p : players) {
						if (p.isAlive()&&(!p.getPid().equals(me.getPid())))
						{
							int acnum=findActionById(me.getPid()).action[state.currentState-State.baseState].raise;
							debug(p.getPid()+" raisenum:"+acnum);
							if(acnum<=mynum)
							{
								debug("bad oppo raise");
								return State.fold;
							}
						}
					}
	...
	public PlayerAction findActionById(String id)
	{
		for(int i=0;i<state.oppotype.size();i++)
		{
			if(state.oppotype.get(i).id.equals(id))
				return state.oppotype.get(i);
		}
		return null;
	}
}
