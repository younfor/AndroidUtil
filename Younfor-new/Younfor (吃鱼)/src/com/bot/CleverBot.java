...
public class CleverBot implements Bot {
	...
	public int getPreAction()
	{
		...
		{
			debug("关煞位");
			boolean isfold=false,israise=false;
			for(Player p:state.players)
			{
				if(p.isAlive()&&(p.getType()==State.bigblind||p.getType()==State.smallblind||p.getType()==State.button))
				{
					if(p.getLastaction()==State.raise&&state.opponent.get(p.getPid()).getPFR()<0.05)
						isfold=true;
					if(p.getLastaction()==State.raise&&state.opponent.get(p.getPid()).getPFR()>0.2)
						israise=true;
					debug(p.getPid()+" PFR:"+state.opponent.get(p.getPid()).getPFR());
				}
				if(p.getPid().equals(me.pid))
				{
					debug("我 PFR:"+state.opponent.get(p.getPid()).getPFR());
				}
			}
			if(isfold)
			{
			...
			}else if(israise)
			{
				...
				if(level>5)
				{
					State.raisebet=(int)(hightobet-prebet);
				}else
					return State.fold;
			}
			if((isFoldAll()))
			{
				State.raisebet=(int)((Math.sqrt(state.getInitjetton()/2000))*4*state.bigblindbet);
				return State.raise;
			}
			if(level>=6)
				return State.call;
