...
public class CleverBot implements Bot {
	...
	public boolean isless(double m)
	{
		if(state.getInitjetton()>m)
			return true;
		else 
			return false;
	}
	...
	public int getPreAction()
	{
		...
		else if(state.myloc==State.button)
		{
			...
			{
				State.raisebet=getSize()*state.bigblindbet;
				return State.raise;
			}
	...
	public int getAction() {
		try {
				...
				}else
				{
					//吓唬
					debug("吓唬");
					if(state.currentState==State.flopState&&hightobet-prebet<=3*state.bigblindbet)
					{
						State.raisebet=getSize()*state.bigblindbet;
						return State.raise;
					}
