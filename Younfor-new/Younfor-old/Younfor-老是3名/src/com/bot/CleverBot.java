	...
	public int getAction() {
		try {
			...
			if (tocall < hightocall) {
				...
				if (base == 0) {
					if(state.currentState!=State.baseState)
						State.raisebet = (int)(0.8*(3+state.callnum) * state.bigblindbet+State.raisebet*0.2);
					return State.raise;
				}
				...
				if ( (state.raisenum < 2)) {
					if(state.currentState!=State.baseState)
						State.raisebet =(int) (0.8*(2+state.callnum+(aveval/base-myval)*3) * state.bigblindbet+State.raisebet*0.2);
					return State.raise;
				} else
					return State.call;
			}
