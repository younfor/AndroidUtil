	...
	public int getAction() {
		try {
			...
			if (tocall < hightocall) {
				...
				{
					if(raisefold&&highbet>state.getInitjetton()/5&&(prob<maxprob+0.1||maxprob==0))
						return State.fold;
					return State.call;
				...
				State.raisebet=(int)Math.max((int)(size*tocall/(1+maxprob)),size*maxaverbet*prob/maxprob);
				//test
				if(state.currentState==State.flopState||state.currentState==State.turnState||state.currentState==State.riverState)
				{
					if(highbet-prebet==0)
					{
						State.raisebet=(int)Math.min((int)(maxaverbet*prob/(0.8+2*maxprob-prob)),state.totalpot*prob);
