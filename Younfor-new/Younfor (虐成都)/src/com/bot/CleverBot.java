	...
	public int getAction() {
		try {
			...
			if (tocall < hightocall) {
				...
				{
					if(highbet<state.getInitjetton()/15&&foldmuch&&(isFoldAll())&&(State.turnState==state.currentState||State.riverState==state.currentState))
					{
				...
				{
					if(highbet<state.getInitjetton()/20&&foldmuch&&(isFoldAll())&&(State.turnState==state.currentState||State.riverState==state.currentState))
					{
						debug("folmuch2 raise");
						State.raisebet=state.totalpot;
						return State.raise;
				...
				if(highbet<state.getInitjetton()/8&&(!raisefold))
				{
					debug("shaby2 raise");
					State.raisebet=2*state.bigblindbet;
					return State.raise;
