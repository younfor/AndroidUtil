	...
	public int getAction() {
		try {
			...
			if (tocall < hightocall) {
				...
				if (base == 0) {
					if(state.currentState!=State.baseState)
						State.raisebet = (int)(0.5*(3+state.callnum) * state.bigblindbet+State.raisebet*0.5);
					//return State.raise;
					return State.call;
				}
				...
				if ( (state.raisenum < 2)) {
					if((state.getInitjetton()>100*state.bigblindbet||me.getGold()<10*state.bigblindbet) && myval>aveval/base)
						return State.call;
					if(state.currentState!=State.baseState)
						State.raisebet =(int) (0.2*(3+state.callnum+(aveval/base-myval)*2.5) * state.bigblindbet+State.raisebet*0.8);
					return State.raise;
				} else
					return State.call;
			}
